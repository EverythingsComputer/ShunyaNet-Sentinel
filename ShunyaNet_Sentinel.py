...
from PySide6.QtWidgets import (
    ...
    QScrollArea, QComboBox, QInputDialog, QFileDialog, QSplitter, QDialog, QTextBrowser, QSizePolicy
)
...
class RSSApp(QWidget):
    def __init__(self):
        ...
        self.settings = {
            ...
            "LLM_BASE_URL": "http://localhost:1234/v1",  # LMStudio default
            "LLM_MODEL": "your-model-name",
            "LLM_API_KEY": "",  # optional
            "SLACK_WEBHOOK_URL": SLACK_WEBHOOK_URL,
            "MAX_INPUT_TOKENS": 4000,
            "MAX_OUTPUT_TOKENS": 1500,
            "MAX_INPUT_TOKENS_BULK": 20000,
            "MAX_OUTPUT_TOKENS_BULK": 5000,
    ...
    def call_llm(self, prompt, max_tokens):
        provider = self.get_setting("LLM_PROVIDER")
        base_url = self.get_setting("LLM_BASE_URL")
        ...
        try:
            if provider in ["lmstudio", "ollama", "llamacpp"]:
                # Both support OpenAI-compatible format
                ...
                self.thread_safe_log(f"Unknown LLM provider: {provider}")
                return None

            if resp.status_code != 200:
                self.thread_safe_log(f"LLM error {resp.status_code}: {resp.text}")
                return None

            return resp.json()["choices"][0]["message"]["content"]

        except Exception as e:
            self.thread_safe_log(f"LLM call failed: {e}")
            return None
        ...
        def save_and_close():
            for name, edit in self.settings_fields.items():
                ...
                if name in [
                    ...
                    "MAX_INPUT_TOKENS",
                    "MAX_OUTPUT_TOKENS",
                    "MAX_INPUT_TOKENS_BULK",
                    "MAX_OUTPUT_TOKENS_BULK",
                    "ITEMS_PER_FEED",
                    "CHUNK_SIZE"
                ]:
                    try:
    ...
    def add_to_history(self, text: str):
        if self._shutting_down:
            return
        try:
            while self.history_layout.count() >= 26:
            ...
            entry_browser = QTextBrowser()
            entry_browser.setHtml(entry_text)
            entry_browser.setOpenExternalLinks(True)  # makes links clickable
            entry_browser.setReadOnly(True)

            entry_browser.setStyleSheet(
                "background-color: #001900; border: 1px solid #00FF41; color: #00FF41; padding: 5px;"
            )
            entry_browser.setMinimumHeight(300)  # or 200 if you prefer
            entry_browser.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)

            self.history_layout.addWidget(entry_browser)
    ...
    # ---------- LMStudio ----------
    def truncate_tokens(self, text, bulk=False):
        if bulk:
            max_tokens = self.get_setting("MAX_INPUT_TOKENS_BULK", int)
        else:
            max_tokens = self.get_setting("MAX_INPUT_TOKENS", int)

        return text[: max_tokens * 4]  # rough token estimate
    # ---------- Slack ----------
    ...
    def fetch_and_send(self):
        try:
            ...
            text_block = self.truncate_tokens(text_block, bulk=False)
            ...
            self.thread_safe_log(f"{len(chunks)} chunk(s) prepared for LLM Analysis.")
            ...
            for idx, chunk in enumerate(chunks):
                ...
                chunk_reply = self.call_llm(
                    prompt_text,
                    self.get_setting("MAX_OUTPUT_TOKENS", int)
                )
                if not chunk_reply:
                    self.thread_safe_log(f"LLM returned no reply for chunk {idx + 1}")
                    continue
                if chunk_reply:
    ...
    def perform_bulk_analysis_if_ready(self):
        try:
            ...
            full_text = self.truncate_tokens(full_text, bulk=True)
            ...
            try:
                reply = self.call_llm(
                    prompt,
                    self.get_setting("MAX_OUTPUT_TOKENS_BULK", int)
                )

                if not reply:
                    self.thread_safe_log("Bulk LLM returned no reply.")
                    return None
